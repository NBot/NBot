...
{
    [Tag("Fun","Meme")]
    public class Akbar : MessageHandler
    {
        private readonly string[] akbars =
        {
        ...
        [Hear("it'?s a trap")]
        [Help(Syntax = "It's a trap", Description = "Returns random Akbar pic", Example = "hey man It's a trap")]
        public void HandleAkbar(Message message, IMessageClient client)
        {
            client.ReplyTo(message, GetRandomItem(akbars));
        }
    }
}