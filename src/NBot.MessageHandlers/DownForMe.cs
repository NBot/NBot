...
namespace NBot.MessageHandlers
{
    ...
    public class DownForMe : MessageHandler
    {
        ...
        [Help(Syntax = "is <site> down for you?", Description = "see if a site is down from the internet.",
            Example = "nbot is www.google.com down for you?")]
        [Help(Syntax = "downforyou <site>", Description = "see if a site is down from the internet.",
            Example = "nbot downforyou www.google.com")]
        public void DoIsItDownForYou(Message message, IMessageClient client, string site)
        {
            try
            {
                ...
                client.ReplyTo(message,
                    match.Success
                        ? string.Format("It's just you. {0} is up for me.", site)
                        : string.Format("Oh no {0} is down for me too!!!!! *PANIC*", site));
            }
            catch
            {
