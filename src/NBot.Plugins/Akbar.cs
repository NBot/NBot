...
{
    public class Akbar : RecieveMessages
    {
        string[] akbars =
            {
        ...
        [RecieveByRegex("it'?s a trap")]
        [Help(Syntax = "It's a trap", Description = "Returns random Akbar pic", Example = "hey man It's a trap")]
        public void DoAkbar(IMessage message, IHostAdapter host)
        {
            host.ReplyTo(message, GetRandomItem(akbars));
        }
    }
}
